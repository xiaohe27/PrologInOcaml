...
(*test if the given path is reachable given the number of rules*)
let rec isPossible path numOfRules =
	match path with
	[] -> true |
	i::tail -> (if i>= numOfRules then false else 
			isPossible tail numOfRules);;

(*If a rule does not fire, then we inc vertically*)
(*otherwise we can add one more element to it.*)
let rec incVert path numOfRules =
	if numOfRules <= 0 then
		None
	else(
	match path with 
	[] -> None |
